...
struct SqrtTree
{
    ...
    inline SqrtTreeItem query(int l, int r, int betweenOffs, int base)
    {
        ...
        if (lBlock <= rBlock)
        {
            SqrtTreeItem add = (layer == 0) ? query(n + lBlock, n + rBlock, (1 << llg) - n, n) : between[layer - 1][betweenOffs + lBound + (lBlock << bCntLog) + rBlock];
            ans = op(ans, add);
        }
    ...
    SqrtTree(const vector<SqrtTreeItem>& a) : n((int)a.size()), llg(log2Up(n)), v(a), clz(1 << llg), onLayer(llg + 1)
    {
        clz[0] = 0;
